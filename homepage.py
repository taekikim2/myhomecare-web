...
# ==========================================
# 6. 관리자 모드 (풀옵션: 블로그 v2.5 + 장부 + 워터마크 + QR)
# ==========================================
elif menu == "🔒 관리자 모드":
    ...
    if password == st.secrets.get("ADMIN_PW", ""):
        ...
        # [탭1] 블로그 글쓰기 (Gemini 2.5 적용됨!)
        with tab1:
            st.subheader("🔥 AI 블로그 파트너 (Gemini 2.5 Flash)")
            st.info("현장 사진을 넣으면 AI가 사진을 보고 글을 써줍니다! (D.I.A+ 로직 적용)")
            ...
            with st.form("blog_form_v2"):
                ...
                context = st.text_area("작업 내용 및 특이사항", height=150, placeholder="예: 아랫집 천장 누수. 공기압 검사하니 온수관 문제. 바닥 깰 수밖에 없었던 이유 등...")
                ...
                if submit_blog:
                    if not topic or not context:
                        ...
                        try:
                            ...
                            [작성 기준: 네이버 최신 알고리즘 반영]
                            1. **C-Rank:** 특정 주제(누수)에 대한 전문성을 강조하십시오.
                            2. **D.I.A+:** 단순 정보 나열 금지. "제가 현장에서 보니~" 같은 독창적 경험과 노하우를 서술하세요.
                            3. **SmartBlock:** '{topic}' 관련 스마트블록에 노출되도록 구체적인 해결책을 제시하세요.
                            ...
                            with st.spinner("AI(2.5 Flash)가 현장 사진을 분석하고 글을 쓰는 중입니다..."):
                                response = model.generate_content(request_content)
                                st.success("작성 완료! 복사해서 쓰세요.")
